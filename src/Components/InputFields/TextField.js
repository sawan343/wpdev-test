...
const TextField = (props) => {
  const { id,isPasswordField,errorText,info, showPassword, isShowPassword, ...restProps } = props;
  ...
  const inputClass = isError ? 'errorBorder' :  "";
   ...
   return (
    <Fragment>
      <input {...restProps} onChange={props.onChange} className ={inputClass} id={id} />
      <label aria-describedby={id} className={labelClasses }>{restProps.placeholder}</label>
      {isPasswordField && (
